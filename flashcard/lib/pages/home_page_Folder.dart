...
Future<void> updateFolderName(String folderId, String newName) async {
  try {
    ...
    setState(() {
      ...
      int index = _folders.indexWhere((folder) => folder.documentId == folderId);
      if (index != -1) {
...
Future<void> _loadData() async {
    ...
    if (user != null) {
      ...
      // Load folders data
      List<Folder> folders = await _userService.getAllFoldersByUserId(widget.userId);
      setState(() {
        _folders = folders;
  ...
  child:  _folders.isEmpty
    ...
    itemBuilder: (BuildContext context, int index) {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => FolderDetailPage(folderId: _folders[index].documentId, userId: widget.userId,),
        ),
      );
    },
        trailing: Row(
          ...
          children: [
            ...
            IconButton(
              ...
              onPressed: () {
                deleteFolder1(_user.id, _folders[index].documentId);
